...
from petstore.services import auth, checkout, fulfillment, inventory  # noqa: E402
from petstore.services.models import CartItem  # noqa: E402
...
def _nav(user: Optional[dict], admin_principal: Optional[Tuple[str, str]]) -> str:
    if admin_principal:
        return '<a href="/admin">Admin Dashboard</a><a href="/admin/logout">Admin Logout</a>'
    if user:
        return (f'<span>Hi, {html.escape(user["name"])}</span>'
                f'<a href="/orders">My Orders</a><a href="/logout">Logout</a>')
    return '<a href="/login">Login</a><a href="/register">Register</a><a href="/admin/login">Admin</a>'
    ...
    if rows:
        ...
        for sku in rows:
            ...
            parts.append(
                ...
                f'<td><a class="btn" style="padding:4px 10px;font-size:11px" '
                f'href="/checkout?sku_id={html.escape(sku.sku_id)}">Buy</a></td></tr>'
            )
        parts.append("</table>")
        body = "".join(parts)
    else:
...
def _login_page(error: Optional[str] = None) -> bytes:
    ...
    """
    return _page("Log In", body, _nav(None, None))


def _register_page(error: Optional[str] = None) -> bytes:
    err = f'<div class="error">{html.escape(error)}</div>' if error else ""
    body = f"""
    <div class="card">
    ...
    """
    return _page("Register", body, _nav(None, None))
...
def _checkout_page(sku_id: str, user: Optional[dict], error: Optional[str] = None) -> bytes:
    sku = inventory.get_sku(sku_id)
    if sku is None:
        return _page("Checkout", '<p class="empty">Unknown SKU.</p>', _nav(user, None))

    err = f'<div class="error">{html.escape(error)}</div>' if error else ""
    guest_note = "" if user else '<p class="muted">Checking out as a guest -- no account needed.</p>'
    gate_note = ('<p class="muted"><strong>Note:</strong> this is a livestock item -- the order will '
                 "wait for admin approval before shipping.</p>" if sku.is_livestock else "")

    image_file = image_for_species(sku.species) if sku.species else None
    img_html = (f'<img src="images/{html.escape(image_file)}" alt="{html.escape(sku.species or "")}" '
                f'style="width:100%;max-width:220px;border-radius:6px;border:1px solid #ccc;'
                f'display:block;margin-bottom:12px">' if image_file else "")

    body = f"""
    <div class="card">
      {img_html}
      <p><strong>{html.escape(sku.name)}</strong> — ${sku.unit_price_cents / 100:.2f}</p>
      <p class="muted">{html.escape(sku.description)}</p>
      {guest_note}
      {gate_note}
      <form method="post" action="/checkout">
        <input type="hidden" name="sku_id" value="{html.escape(sku.sku_id)}">
    ...
    """
    return _page("Checkout", body, _nav(user, None))


def _order_confirmation_page(result, user: Optional[dict]) -> bytes:
    ...
    body += '<a class="back" href="index.html">&larr; Back to storefront</a>'
    return _page("Order Confirmation", body, _nav(user, None))


def _order_status_page(order_id: str, user: Optional[dict]) -> bytes:
    ...
    if status is None:
        ...
        """
    body += '<a class="back" href="index.html">&larr; Back to storefront</a>'
    return _page("Order Status", body, _nav(user, None))


def _orders_page(user: dict) -> bytes:
    ...
    if not history:
        ...
        rows.append("</table>")
        body = "".join(rows)
    return _page("My Orders", body, _nav(user, None))
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/category":
            self._send_html(_render_category(query.get("category_id", [""])[0], _nav(user, admin)))
            return
        if path == "/login":
            self._send_html(_login_page())
            return
        if path == "/register":
            self._send_html(_register_page())
            return
        if path == "/logout":
            ...
            return
        if path == "/checkout":
            self._send_html(_checkout_page(query.get("sku_id", [""])[0], user))
            return
        if path == "/orders":
            if not user:
                self._redirect("/login")
                return
            self._send_html(_orders_page(user))
            return
        if path == "/order-status":
            self._send_html(_order_status_page(query.get("order_id", [""])[0], user))
            return
        if path == "/admin/login":
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if path == "/login":
            ...
            if token is None:
                self._send_html(_login_page(error="Invalid email or password."), status=401)
                return
        ...
        if path == "/register":
            try:
                ...
                self._send_html(_register_page(error=str(exc)), status=400)
                return
        ...
        if path == "/checkout":
            ...
            result = checkout.place_order(
                cart=[CartItem(sku_id, quantity)],
                customer_id=customer_id,
                payment_method=form.get("payment_method", ""),
                user_id=user["user_id"] if user else None,
            )
            self._send_html(_order_confirmation_page(result, user))
            return
